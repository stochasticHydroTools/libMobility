/* Raul P. Pelaez 2021. Doubly Periodic Stokes UAMMD wrapper
   ...
   Additionally, a glue class is provided to ease separate compilation between
   GPU code (this source) and another code. For instance, the python wrapper in
   uammd_python.cpp
*/
#include <uammd.cuh>
...
public:
  ...
  void setPositions(const real *d_pos, int numberParticles) {
    ...
    thrust::copy(
        thrust::cuda::par.on(st), reinterpret_cast<const uammd::real3 *>(d_pos),
  ...
  // Compute the hydrodynamic displacements due to a series of forces and/or
  ...
  void Mdot(const real *h_forces, const real *h_torques, real *h_MF, real *h_MT,
    ...
    if (givenForces) {
      ...
      thrust::fill(thrust::cuda::par.on(st), force4.begin(), force4.end(),
    ...
    thrust::transform(thrust::cuda::par.on(st), stored_positions_real3.begin(),
                      stored_positions_real3.end(), stored_positions.begin(),
...
// Compute the dot product of the mobility matrix with the forces and/or torques
// acting on the previously provided positions
void DPStokesGlue::Mdot(const real *h_forces, const real *h_torques, real *h_MF,
...
void DPStokesGlue::throwIfInvalid() {
  if (not dpstokes) {
    throw std::runtime_error(
        "DPStokes is not initialized. Call Initialize first");
  }
}
